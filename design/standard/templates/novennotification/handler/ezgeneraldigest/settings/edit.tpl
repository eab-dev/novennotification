{* Modified version of extension/ezwebin/design/ezwebin/templates/notification/handler/ezgeneraldigest/settings/edit.tpl to allow us to inject $settings variable *}

<div class="block">
    <label><input type="checkbox" name="ReceiveDigest_{$handler.id_string}" {$settings.receive_digest|choose( '', 'checked="checked"')} /> {'Receive all messages combined in one digest'|i18n( 'design/ezwebin/notification/handler/ezgeneraldigest/settings/edit' )}</label>
</div>
