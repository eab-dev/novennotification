{ezscript_require( array( 'tools/ezjsselection.js' ) )}
    ...
    <div class="box-mr">
        <div class="box-content">
            <div class="context-attributes">
                {if $user}
                <div class="object">
                    <h2>{$user.contentobject.name} - {$user.login} - <a href="mailto:{$user.email}">{$user.email}</a></h2>
                </div>
                {/if}
		...
		{* DESIGN: Content END *}
                <div class="buttonblock">
                {if $subscribed_nodes_count}
                <input class="button" type="submit" name="RemoveRule_{$remove_id}" value="{'Remove selected'|i18n( 'design/ezwebin/templates/novennotification/view' )}" />
                {/if}
                </div>
            </form>
        </div>
